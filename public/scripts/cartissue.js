...
cartIssueButton.addEventListener('click',async ()=>{
    ...
    await fetch('http://localhost:5001/issue/'+currentusername.textContent,{body:JSON.stringify({books:data.bookid}),method:"POST",headers: { 'Content-Type': 'application/json' }});
...
issueButtons.forEach((x)=>{
    ...
    await fetch('http://localhost:5001/issue/'+currentusername.textContent,{body:JSON.stringify({books:[abookId]}),method:"POST",headers: { 'Content-Type': 'application/json' }});
